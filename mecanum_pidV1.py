...
import numpy as np
from my_package_py.pid_controller import PIDController
from my_package_py.mecanum_kinematic import Mecanum
...
class PID_Simulation(Node):
    def __init__(self):
        ...
        timer = 0.01
        self.output_pub = self.create_publisher(Twist,'/pointer',10)
        ...
        ## Variable PID
        self.dt = 0.01
        self.Kp = np.array([0.80, 0.8, 0.6])
        self.Ki = np.array([0.0, 0.0, 0.0])
        self.Kd = np.array([0.04, 0.04, 0.02])
        self.alpha = 0.7
        self.integral_min = -1.2
        self.integral_max = 1.2
        self.output_min = -1.2
        self.output_max = 1.2
    ...
    def Calculate_position(self):
        ...
        self.get_logger().info('Current_state : "[%f, %f, %f]"' % (self.current_x, self.current_y, self.current_yaw))
